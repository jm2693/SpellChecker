...
//                 if (c == '-') {
//                     hyphenated_word = 1;
//                     word_buffer[word_index] = '\0'; // include the hyphen
//                     word_index++;
//                     continue;
//                         // Word key;
...
//             if (hyphenated_word) {
//                 return_error(txt_file, punct_string, line_counter, column_counter - (word_index+punct)); // Adjust column for incomplete word
//             } else return_error(txt_file, punct_string, line_counter, column_counter - (word_index+punct));
//         }
//     }
...
char *trim_punc(char *str)
{
  char *end;

  // Trim trailing space
  end = str + strlen(str) - 1;
  while(end > str && ispunct((unsigned char)*end)) end--;
...
void check_spelling(char* txt_file, Word* dictionary, Word* dictionary1, Word* dictionary2, int word_num) {
    ...
    while ((bytes_read = read(fd, buffer, sizeof(buffer))) > 0) {
        ...
        for (ssize_t i = 0; i < bytes_read; i++) {
            ...
            char c = buffer[i];
            column_counter++;
            if (ispunct(c)) trail = 1;
            if (isalpha(c)) trail = 0;
            if (c == ' ' || c == '\n' || c == '\t' || c == '\v' || c == '-') {
                ...
                if (c == '-') {
                    word_buffer[word_index] = '\0'; // null-terminate the word
                    if (trail == 1) {
                        trim_punc(word_buffer);
                    }
                    if (word_buffer[word_index] == ' ') continue;
                    Word key;
                    key.word = word_buffer;
                    strcat(punct_string, word_buffer);

                    // while (c != ' ' && c != '\n' && c != '\t' && c != '\v') {
                    //     punct_string[punct + word_index - 1] = '-';
                    //     if (word_index < WORD_LENGTH-1) {
                    //         word_buffer[(word_index++)] = c;
                    //     }
                ...
                if (word_index > 0) {
                    ...
                    if (word_buffer[word_index-1] == '.') word_buffer[word_index-1] = '\0';
                    Word key;
                    key.word = word_buffer;
                    strcat(punct_string, word_buffer);
                ...
                if (word_index < WORD_LENGTH-1) {
                    // trail = 0;
                    // no_trail = &word_buffer[word_index];
                    word_buffer[(word_index++)] = c;
                }
            }
